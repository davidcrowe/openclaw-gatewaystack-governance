...
import {
  ...
  INJECTION_PATTERNS_EXTRACTION,
  INJECTION_PATTERNS_MULTILANG,
} from "./constants.js";
 ...
 */
export function deobfuscate(input: string): DecodedCandidate[] {
...
export function checkCanaryTokens(
  input: string,
  tokens: string[]
): string[] {
  const matches: string[] = [];
...
export function detectInjection(
  ...
  matches.push(...scanPatterns(args, INJECTION_PATTERNS_HIGH, "HIGH"));
  matches.push(...scanPatterns(args, INJECTION_PATTERNS_EXTRACTION, "HIGH"));

  if (sensitivity === "medium" || sensitivity === "high") {
    matches.push(...scanPatterns(args, INJECTION_PATTERNS_MEDIUM, "MEDIUM"));
  }

  if (sensitivity === "high") {
    matches.push(...scanPatterns(args, INJECTION_PATTERNS_LOW, "LOW"));
  }

  // Phase 2: Multi-language scan
  if (policy.injectionDetection.multiLanguage !== false) {
    matches.push(...scanPatterns(args, INJECTION_PATTERNS_MULTILANG, "MULTILANG"));
  }
  ...
  if (policy.injectionDetection.obfuscationDetection !== false) {
    ...
    for (const candidate of decoded) {
      ...
      matches.push(...scanPatterns(candidate.decoded, INJECTION_PATTERNS_HIGH, prefix));
      matches.push(...scanPatterns(candidate.decoded, INJECTION_PATTERNS_EXTRACTION, prefix));
      if (sensitivity === "medium" || sensitivity === "high") {
        matches.push(...scanPatterns(candidate.decoded, INJECTION_PATTERNS_MEDIUM, prefix));
      }
      if (policy.injectionDetection.multiLanguage !== false) {
        matches.push(...scanPatterns(candidate.decoded, INJECTION_PATTERNS_MULTILANG, prefix));
      }
    }
  }
  ...
  if (
    policy.injectionDetection.canaryTokens &&
